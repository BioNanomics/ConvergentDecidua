...
- **⚠️ Cross-species caveat (active investigation — see
  ...
  **0 %** in mouse, despite confirmed 1:1 orthology and presence in the
  mouse var set. Three candidate explanations, ordered by likelihood:
  1. **Symbol / case mismatch survives the backbone remap** in the mouse
     processed h5ad (e.g. raw symbol is `Igfbp1` but our remap step is
     case-sensitive somewhere). Highest probability. Must be ruled out
     before any biological inference.
  ...
  3. **True biological divergence.** The mouse decidualization secretome
     emphasises the *Prl* family (`Prl8a2`, `Prl3c1`, `Prl3d1`) rather
     than IGFBP1 as the dominant secreted product; some loss of IGFBP1
     dominance in mouse stroma is biologically real, but a strict 0 %
     value is suspicious and almost certainly reflects (1) or (2).
...
| Gene   | 1:1 orthology | Functional ortholog confirmed | Open caveat |
|--------|:------:|:------:|---|
| PGR    | ✅ | ✅ (Lydon 1995, mouse cKO + human HESC) | — |
| FOXO1  | ✅ | ✅ (Christian 2002 human, Vasquez 2018 mouse) | timing of PI3K/AKT regulation may differ |
| HAND2  | ✅ | ✅ (Li 2011 mouse, Huyen 2011 + Marinić 2021 comparative) | — |
| WNT4   | ✅ | ✅ (Franco 2011 mouse cKO + human HESC) | family co-expression — module-level only |
| IGFBP1 | ✅ | ✅ in human; ⚠️ mouse pipeline finding (0 %) | symbol/stage check required before any IGFBP1-based cross-species claim |
| IL15   | ✅ | ✅ (Ashkar 2003 mouse; Verma 2000 human) | downstream-cell-fate claims need uNK data |

**Bottom line.** Sequence orthology for the protected core is solid. Of
the six, **five (PGR, FOXO1, HAND2, WNT4, IL15) are safe to centre
year-one comparative claims on without further work**. **IGFBP1** has a
1:1 ortholog and human evidence, but the mouse 0 % expression needs to
be diagnosed (symbol / stage / true divergence) before IGFBP1 carries
any cross-species claim — track in `marker_recovery_plan.md`.

This memo discharges pre-Q3 acceptance gate item C in [`../PLAN.md`](../PLAN.md).
